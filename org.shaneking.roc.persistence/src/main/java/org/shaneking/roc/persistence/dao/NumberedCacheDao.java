...
public class NumberedCacheDao {
  ...
  public <T extends NumberedEntity> T oneByNo(@NonNull Class<T> cacheType, @NonNull String no, @NonNull String tenantId) {
    return oneByNo(cacheType, no, tenantId, true);
  }

  public <T extends NumberedEntity> T oneByNo(@NonNull Class<T> cacheType, @NonNull String no, @NonNull String tenantId, boolean rtnNullIfNotEqualsOne) {
    T rtn = null;
  ...
  private <T extends NumberedEntity> boolean eq(@NonNull String no, @NonNull String tenantId, T t) {
    return t != null && no.equals(t.getNo()) && tenantId.equals(t.getTenantId());
  }

  private <T extends NumberedEntity> T oneByNo(@NonNull Class<T> cacheType, @NonNull String no, @NonNull String tenantId, boolean rtnNullIfNotEqualsOne, String key) {
    T rtn = null;
    try {
      ...
      log.error(OM3.lp(rtn, cacheType.getName(), no, tenantId, rtnNullIfNotEqualsOne));
    }
    return rtn;
  }
}
