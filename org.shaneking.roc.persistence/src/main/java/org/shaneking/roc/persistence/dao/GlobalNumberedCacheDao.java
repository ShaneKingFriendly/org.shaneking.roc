...
@Repository
@Slf4j
public class GlobalNumberedCacheDao {
  @Autowired
  @Getter
  ...
  public <T extends GlobalNumberedEntity> T oneByNo(@NonNull Class<T> cacheType, @NonNull String no) {
    return oneByNo(cacheType, no, true);
  }

  public <T extends GlobalNumberedEntity> T oneByNo(@NonNull Class<T> cacheType, @NonNull String no, boolean rtnNullIfNotEqualsOne) {
    T rtn = null;
  ...
  private <T extends GlobalNumberedEntity> boolean eq(@NonNull String no, T t) {
    return t != null && no.equals(t.getNo());
  }

  private <T extends GlobalNumberedEntity> T oneByNo(@NonNull Class<T> cacheType, @NonNull String no, boolean rtnNullIfNotEqualsOne, String key) {
    T rtn = null;
    try {
